...
class Phone(Field):
    def __init__(self, value):
        if not re.fullmatch(r'\d{10}', value):
            raise ValueError(f"Phone number {value} should consist of 10 digits")
        else:
            self.value = value
...
#метод для редагування телефону, якщо телефон знайдено то змінюємо його по індексу    
    def edit_phone(self, old:str, new:str):
        old_phone = Phone(old)
        new_phone = Phone(new)
        if self.find_phone(old_phone.value)[0]:
            i = self.find_phone(old_phone.value)[1]
            self.phones[i]=new_phone
        else:
            print(f"Contact {self.name} has no such phone number: {old}")
